...
4. Add a `languageName_<code>` key: this is the human-readable name
   ...
   For a new `ca` block: `languageName_ca: "català"`. Add it to **every
...
- **`situacion` is a shared key across every language.** Pick from
  ...
  `trabajo`, `legal`, `tecnologia`, `seguridad`, `educacion`. These ten
  categories are the **IADL areas** (see [`SPEC.md`](SPEC.md)), the framework
  ...
  **not** invent a new one for a handful of words; see "When to add a
  ...
  handful of words that justifies it, and a `topic_<key>` label added
...
(`htmlLang`, `metaTitle`, `metaDescription`) — the full `I18N`
...
1. Add a `<code>: { htmlLang, metaTitle, metaDescription }` entry
...
- The new key is added to the same `situacion` field in **every**
  ...
  `topic_<key>` label is added in **every** `I18N` block, all in the
  same commit. The ten existing keys (`tramites`, `salud`,
  `vida-diaria`, `finanzas`, `vivienda`, `trabajo`, `legal`,
  `tecnologia`, `seguridad`, `educacion`) are the AIVD framework — they apply to
