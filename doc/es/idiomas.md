...
4. Añade una clave `languageName_<código>`: es el nombre legible que
   el idioma nuevo usa para sí mismo en el enlace cruzado **"Ver en
   {idioma}: {palabra}"**. Hoy:
   - `languageName_es: "español"`
   - `languageName_en: "inglés"`
   Para un nuevo bloque `ca`: `languageName_ca: "català"`. Añádelo
...
- **`situacion` es una clave compartida por todos los idiomas.**
  ...
  `vivienda`, `trabajo`, `legal`, `tecnologia`, `seguridad`,
  `educacion`. Estas diez categorías son las **áreas AIVD** (ver [`SPEC.md`](SPEC.md)),
  ...
  que la justifique, y una etiqueta `topic_<clave>` añadida en
...
relevantes para `<head>` (`htmlLang`, `metaTitle`,
`metaDescription`) — la copia completa de `I18N` vive en
...
1. Añade una entrada
   `<código>: { htmlLang, metaTitle, metaDescription }` a
...
- La clave nueva se añade al campo `situacion` de las entradas
  ...
  los idiomas, y se añade una etiqueta `topic_<clave>` en **cada**
  bloque `I18N`, todo en el mismo commit. Las diez claves
  existentes (`tramites`, `salud`, `vida-diaria`, `finanzas`,
  `vivienda`, `trabajo`, `legal`, `tecnologia`, `seguridad`,
  `educacion`) son el marco AIVD — aplican a todos los idiomas porque describen
